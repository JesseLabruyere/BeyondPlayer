...
$(document).ready(function() {
    ...
    $scope.functions = {}, $scope.songQueue = [], $scope.selected = {}, $scope.future = [], 
    $scope.past = [];
    var player;
    $scope.functions.loadQueue = function(item, event) {}, $scope.functions.loadQueue(), 
    $scope.$on("queue", function() {
        $scope.songQueue = angular.copy(sharedService.songQueue), !$scope.songQueue.length > 0 || ($scope.future = angular.copy($scope.songQueue), 
        $scope.selected = $scope.future.splice(0, 1)[0], $scope.selected.fullPath = $scope.selected.uploadDirectory + "/" + $scope.selected.path, 
        $timeout(function() {
            player.setSrc($scope.selected.fullPath), player.play();
        }));
    }), $scope.functions.nextSong = function(item, event) {
        if ($scope.past.push($scope.selected), $scope.future.length > 0) ; else {
            if (!($scope.past.length > 0)) return;
            $scope.future = $scope.past, $scope.past = [];
        }
        $scope.selected = $scope.future.splice(0, 1)[0], $scope.selected.fullPath = $scope.selected.uploadDirectory + "/" + $scope.selected.path, 
        $timeout(function() {
            player.setSrc($scope.selected.fullPath), player.play();
