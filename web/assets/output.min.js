...
app.directive("leftMenuDirective", function($q) {
    var linkFn = function($scope, $http) {
        $scope.functions = {}, $scope.functions.loadUploadPage = function(item, event) {
            ...
            $q.all({
                view: $http.get("app/getPlaylistView").error(function(data, status, headers, config) {
                    alert("AJAX failed!");
                }),
                data: $http.get("app/getPlaylists").error(function(data, status, headers, config) {
                    alert("AJAX failed!");
                })
            }).then(function(results) {
                var view = results.view.data, data = results.data.data;
                $("#pageCenter").html(view);
                var playlists = data.playlists;
                void 0 !== playlists && data.success && renderPlaylistData(playlists);
            });
        };
    };
    return {
