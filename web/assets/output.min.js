function assignFormHandlers() {
    $(document).on("submit", "#upload_form", function(e) {
        e.preventDefault(), $("#upload_form").ajaxSubmit(function(response) {
            response = JSON.parse(response), response.success ? (alert("Thank you for your upload!"), 
            $("#upload_form")[0].reset()) : alert(response.reason);
        });
    }), $(document).on("submit", "#registration_form", function(e) {
        e.preventDefault(), $("#registration_form").ajaxSubmit(function(response) {
            response = JSON.parse(response), response.success ? (alert("Thank you for your registration"), 
            $("#registration_form")[0].reset()) : alert(response.reason);
        });
    });
}
...
$(document).ready(function() {}), app.factory("sharedService", function($rootScope, $q, $http) {
    var sharedService = {};
    return sharedService.songQueue = [], sharedService.songToQueue = {}, sharedService.playlists = [], 
    sharedService.broadcast = function(message, object) {
        "undefined" == typeof object && (object = {}), this.broadcastItem(message, object);
        ...
        response.success(function(data, status, headers, config) {
            void 0 !== data.playlists && data.success && (sharedService.playlists = data.playlists);
        }), response.error(function(data, status, headers, config) {
            alert("Connection to server failed!");
        });
    }, sharedService.getPlaylists(), sharedService.setSongQueue = function(data) {
        sharedService.songQueue = data, this.broadcast("queue");
    }, sharedService.addToQueue = function(song) {
        sharedService.songToQueue = song, this.broadcast("addToQueue");
    }, sharedService.playSong = function(song) {
        sharedService.songToQueue = song, this.broadcast("playSong");
    }, sharedService.removeSong = function(id) {
        var deferred = $q.defer(), response = $http.get("app/removeAudio/" + id);
        return response.success(function(data, status, headers, config) {
            data.success ? (deferred.resolve(data.success), alert("Song removed")) : (deferred.resolve(!1), 
            alert("Could not be removed"));
        }), response.error(function(data, status, headers, config) {
            deferred.resolve(!1), alert("Connection to server failed!");
        }), deferred.promise;
    }, sharedService.addToList = function(listId, songId) {
        var deferred = $q.defer(), response = $http.get("app/addToPlaylist/" + listId + "/" + songId);
        return response.success(function(data, status, headers, config) {
            data.success ? alert("Added to playlist") : "duplicate" == data.reason && alert("The playlist already contains this item");
        }), response.error(function(data, status, headers, config) {
            alert("Connection to server failed!");
        }), deferred.promise;
    }, sharedService.paginationInfo = {}, sharedService.initiatePagination = function(totalItems, pageNumber, url, numPages, itemsPerPage) {
        sharedService.paginationInfo.totalItems = totalItems, sharedService.paginationInfo.currentPage = "undefined" != typeof pageNumber ? pageNumber : 1, 
        sharedService.paginationInfo.url = url, sharedService.paginationInfo.numPages = numPages, 
        sharedService.paginationInfo.itemsPerPage = itemsPerPage, sharedService.paginationInfo.maxSize = 5, 
        sharedService.paginationInfo.paginationShown = !0, this.broadcastItem("initiatePagination");
    }, sharedService.disablePagination = function() {
        sharedService.paginationInfo.paginationShown = !1, this.broadcastItem("disablePagination");
    }, sharedService;
...
}), app.controller("uploadController", function($scope) {}), app.controller("uploadsController", function($scope, $http, $timeout, $routeParams, $location, sharedService) {
    $scope.functions = {}, $scope.playlists = [], $scope.itemsPerPage = 25, $scope.currentPage = $routeParams.page, 
    $scope.pages = 0, $scope.functions.loadUploadsView = function(item, event) {
        if ("undefined" == typeof $scope.currentPage) var response = $http.get("app/user/getUploads"); else var response = $http.get("app/user/getUploads/" + $scope.currentPage + "/" + $scope.itemsPerPage);
        response.success(function(data, status, headers, config) {
            void 0 !== data.playlist && data.success && ($scope.songs = data.playlist.listItems, 
            $scope.playlists = sharedService.playlists, "undefined" != typeof $scope.currentPage && sharedService.initiatePagination(data.playlist.listCount, $scope.currentPage, "/uploads", data.playlist.pages, data.playlist.itemsPerPage)), 
            $timeout(function() {});
        }), response.error(function(data, status, headers, config) {
            alert("AJAX failed!");
        });
    }, $scope.functions.loadUploadsView(), $scope.functions.loadPage = function(pageNumber) {
        var response = $http.get("app/user/getUploads/" + pageNumber + "/" + $scope.itemsPerPage);
        response.success(function(data, status, headers, config) {
            "undefined" != typeof data.playlist && data.success ? ($scope.currentPage = pageNumber, 
            $scope.songs = data.playlist.listItems, $scope.playlists = sharedService.playlists) : !data.success && "page does not exist" === data.reason, 
            "undefined" != typeof data.playlist && data.playlist.pages != $scope.pages && ($scope.pages = data.playlist.pages, 
        ...
        $routeParams.page != $scope.currentPage && "undefined" != typeof $routeParams.page ? $scope.functions.loadPage($routeParams.page) : "undefined" != $routeParams.page && (delete $scope.currentPage, 
        ...
        sharedService.removeSong(id).then(function(success) {
            success && $scope.functions.removeItemFromData(index);
        });
...
}), app.controller("registrationController", function($scope) {}), app.controller("playlistsController", function($scope, $http) {
    $scope.functions = {}, $scope.functions.loadPlaylistsView = function(item, event) {
        var response = $http.get("app/user/getPlaylists");
        response.success(function(data, status, headers, config) {
            void 0 !== data.playlists && data.success && ($scope.playlists = data.playlists);
        }), response.error(function(data, status, headers, config) {
            alert("AJAX failed!");
        });
    }, $scope.functions.loadPlaylistsView();
}), app.controller("playlistController", function($scope, $http, $routeParams, $timeout, sharedService) {
    $scope.functions = {}, $scope.playlists = [], $scope.functions.loadPlaylistView = function(item, event) {
        var response = $http.get("app/user/getPlaylistResults/" + $routeParams.playlistId);
        response.success(function(data, status, headers, config) {
            void 0 !== data.playlist && data.success && ($scope.songs = data.playlist.listItems, 
            $scope.playlists = sharedService.playlists), $timeout(function() {});
        }), response.error(function(data, status, headers, config) {
            alert("AJAX failed!");
        });
    }, $scope.functions.loadPlaylistView(), $scope.functions.playPlaylist = function() {
    ...
    $scope.functions = {}, $scope.functions.loadAlbumsView = function(item, event) {
        var response = $http.get("app/user/getAlbums");
        response.success(function(data, status, headers, config) {
            void 0 !== data.albums && data.success && ($scope.albums = data.albums);
        }), response.error(function(data, status, headers, config) {
            alert("AJAX failed!");
        });
    }, $scope.functions.loadAlbumsView();
}), app.controller("albumController", function($scope, $http, $routeParams) {
    $scope.functions = {}, $scope.functions.loadPlaylistView = function(item, event) {
        var response = $http.get("app/user/getAlbumResults/" + $routeParams.albumId);
        response.success(function(data, status, headers, config) {
            void 0 !== data.album && data.success && ($scope.songs = data.album.listItems);
        }), response.error(function(data, status, headers, config) {
            alert("AJAX failed!");
        });
    }, $scope.functions.loadPlaylistView();
}), app.controller("footerController", function($scope, $http, $routeParams, $timeout, $q, sharedService) {
    $scope.functions = {}, $scope.songQueue = [], $scope.selected = {}, $scope.highLighted = {}, 
    $scope.lastId = 0;
    var player;
    $scope.functions.loadQueue = function(item, event) {}, $scope.functions.loadQueue(), 
    $scope.$on("queue", function() {
        var array = angular.copy(sharedService.songQueue);
        $scope.songQueue = $scope.functions.generateIds(array), !$scope.songQueue.length > 0 || ($scope.selected = $scope.songQueue[0], 
        $timeout(function() {
            player.setSrc($scope.selected.fullPath), player.play();
        }));
    }), $scope.$on("addToQueue", function() {
        var song = angular.copy(sharedService.songToQueue);
        song = $scope.functions.generateId(song), $scope.songQueue.push(song), 1 === $scope.songQueue.length && ($scope.selected = $scope.songQueue[0], 
        $timeout(function() {
            player.setSrc($scope.selected.fullPath), player.play();
        }));
    }), $scope.$on("playSong", function() {
        var song = angular.copy(sharedService.songToQueue);
        if (song = $scope.functions.generateId(song), $scope.songQueue.length > 0) {
            var currentIndex = $scope.functions.findSongIndex($scope.selected.generatedId);
            $scope.songQueue.splice(currentIndex, 0, song), $scope.selected = song, $scope.highLighted = song;
        } else $scope.songQueue.push(song), $scope.selected = song, $scope.highLighted = song;
        $timeout(function() {
            player.setSrc($scope.selected.fullPath), player.play();
        });
    }), $scope.functions.nextSong = function(item, event) {
        var deferred = $q.defer(), currentIndex = $scope.functions.findSongIndex($scope.selected.generatedId);
        return currentIndex + 1 == $scope.songQueue.length ? $scope.selected = $scope.songQueue[0] : $scope.selected = $scope.songQueue[currentIndex + 1], 
        $timeout(function() {
            player.setSrc($scope.selected.fullPath), player.play();
        }), deferred.resolve(), deferred.promise;
    }, $scope.functions.playSong = function(songIndex) {
        songIndex + 1 > $scope.songQueue.length || ($scope.selected = $scope.songQueue[songIndex], 
        $timeout(function() {
            player.setSrc($scope.selected.fullPath), player.play();
        }));
    }, $scope.functions.buildMediaPlayer = function() {
        return player = new MediaElementPlayer("#audioPLayer", {
        ...
        for (var i = 0; i < array.length; i++) $scope.lastId++, array[i].generatedId = $scope.lastId;
        return array;
    }, $scope.functions.generateId = function(object) {
        return $scope.lastId++, object.generatedId = $scope.lastId, object;
    }, $scope.functions.findSongIndex = function(generatedId) {
        for (var i = 0; i < $scope.songQueue.length; i++) if ($scope.songQueue[i].generatedId === generatedId) return i;
    }, $timeout(function() {
        $scope.functions.buildMediaPlayer();
    }), $scope.functions.songClicked = function(index) {
        return $scope.justDragged === index ? void ($scope.justDragged = 99999) : void ($scope.highLighted.generatedId === $scope.songQueue[index].generatedId && $scope.highLightedThisClick === !1 && $scope.selected.generatedId !== $scope.songQueue[index].generatedId && $scope.functions.playSong(index));
    }, $scope.functions.songMouseDown = function(song) {
        $scope.highLighted.generatedId === song.generatedId ? $scope.highLightedThisClick = !1 : ($scope.highLighted = song, 
        $scope.highLightedThisClick = !0);
    }, $scope.functions.removeFromQueue = function(songIndex) {
        $scope.index === songIndex ? $scope.songQueue.length > 1 ? ($scope.songQueue.splice(songIndex, 1), 
        $scope.index--, $scope.functions.nextSong().then(function() {
            $timeout(function() {
                $scope.indexHighLighted = $scope.index;
            });
        })) : $scope.functions.emptyQueue() : (songIndex + 1 == $scope.songQueue.length && ($scope.indexHighLighted = 0), 
        $scope.songQueue.splice(songIndex, 1), songIndex < $scope.index && $scope.index--);
    }, $scope.functions.emptyQueue = function() {
        $scope.songQueue = [], $scope.selected = {}, $scope.highLighted = {}, player.pause(), 
        player.setSrc("");
    }, $scope.justDragged = 99999, $scope.sortableOptions = {
        start: function(e, ui) {
    ...
    $scope.functions = {}, $scope.totalItems = 50, $scope.currentPage = 1, $scope.url = "", 
    $scope.numPages = 20, $scope.itemsPerPage = 25, $scope.maxSize = 5, $scope.paginationShown = !1, 
    $scope.functions.selectPage = function() {
        $scope.functions.pageChanged();
    }, $scope.functions.pageChanged = function() {
        $location.path($scope.url).search({
            page: $scope.currentPage
        });
    }, $scope.$on("initiatePagination", function() {
        $scope.totalItems = sharedService.paginationInfo.totalItems, $scope.currentPage = sharedService.paginationInfo.currentPage, 
        $scope.url = sharedService.paginationInfo.url, $scope.numPages = sharedService.paginationInfo.numPages, 
        $scope.itemsPerPage = sharedService.paginationInfo.itemsPerPage, $scope.maxSize = sharedService.paginationInfo.maxSize, 
