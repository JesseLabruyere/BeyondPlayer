...
$(document).ready(function() {}), app.factory("sharedService", function($rootScope, $q, $http) {
    ...
    $scope.functions = {}, $scope.playlists = [], $scope.functions.loadUploadsView = function(item, event) {
    ...
    $scope.functions = {}, $scope.songQueue = [], $scope.selected = {}, $scope.index = 0, 
    $scope.indexHighLighted = 0;
    var player;
    $scope.functions.loadQueue = function(item, event) {}, $scope.functions.loadQueue(), 
    $scope.$on("queue", function() {
        $scope.songQueue = angular.copy(sharedService.songQueue), !$scope.songQueue.length > 0 || ($scope.selected = $scope.songQueue[$scope.index], 
        $timeout(function() {
            player.setSrc($scope.selected.fullPath), player.play();
        }));
    }), $scope.$on("addToQueue", function() {
        $scope.songQueue.push(angular.copy(sharedService.songToQueue)), 1 === $scope.songQueue.length && ($scope.selected = $scope.songQueue[0], 
        $timeout(function() {
            player.setSrc($scope.selected.fullPath), player.play();
        }));
    }), $scope.$on("playSong", function() {
        $scope.songQueue.length > 0 ? ($scope.songQueue.splice($scope.index, 0, angular.copy(sharedService.songToQueue)), 
        $scope.selected = $scope.songQueue[$scope.index]) : ($scope.songQueue.push(angular.copy(sharedService.songToQueue)), 
        $scope.selected = $scope.songQueue[0]), $timeout(function() {
            player.setSrc($scope.selected.fullPath), player.play();
        });
    }), $scope.functions.nextSong = function(item, event) {
        var deferred = $q.defer();
        return $scope.index + 1 == $scope.songQueue.length ? $scope.index = 0 : $scope.index++, 
        $scope.selected = $scope.songQueue[$scope.index], $timeout(function() {
            player.setSrc($scope.selected.fullPath), player.play();
        }), deferred.resolve(), deferred.promise;
    }, $scope.functions.playSong = function(songIndex) {
        songIndex + 1 > $scope.songQueue.length || ($scope.index = songIndex, $scope.selected = $scope.songQueue[$scope.index], 
        $timeout(function() {
            player.setSrc($scope.selected.fullPath), player.play();
        }));
        ...
        return $scope.justDragged === songIndex ? void ($scope.justDragged = 99999) : void ($scope.indexHighLighted === songIndex && $scope.highLightedThisClick === !1 && $scope.index !== songIndex && $scope.functions.playSong(songIndex));
    }, $scope.functions.songMouseDown = function(songIndex) {
        $scope.indexHighLighted === songIndex ? $scope.highLightedThisClick = !1 : ($scope.indexHighLighted = songIndex, 
        ...
        $scope.songQueue = [], $scope.selected = {}, $scope.index = 0, $scope.indexHighLighted = 0, 
        player.pause(), player.setSrc("");
    }, $scope.oldIndex = 0, $scope.justDragged = 99999, $scope.sortableOptions = {
        start: function(e, ui) {
            $scope.oldIndex = ui.item.sortable.index;
        },
        stop: function(e, ui) {
            $scope.index == $scope.oldIndex && ($scope.index = ui.item.index()), $scope.index !== ui.item.index() && (ui.item.index() > $scope.index && $scope.oldIndex < $scope.index && 0 !== $scope.index ? $scope.index-- : ui.item.index() < $scope.index && $scope.oldIndex > $scope.index && $scope.index++), 
            $scope.indexHighLighted = ui.item.index(), $scope.justDragged = ui.item.index(), 
            $scope.isPLaying = !1;
        }
    };
});