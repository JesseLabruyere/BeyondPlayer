...
$(document).ready(function() {}), app.factory("sharedService", function($rootScope, $q, $http) {
    ...
    $scope.functions = {}, $scope.playlists = [], $scope.functions.loadUploadsView = function(item, event) {
        ...
        response.success(function(data, status, headers, config) {
            void 0 !== data.uploads && data.success && ($scope.songs = data.uploads.listItems, 
    ...
    $scope.functions = {}, $scope.functions.loadPlaylistView = function(item, event) {
        ...
        response.success(function(data, status, headers, config) {
            void 0 !== data.album && data.success && ($scope.songs = data.album.tracks);
        }), response.error(function(data, status, headers, config) {
            alert("AJAX failed!");
        });
    ...
    }, $scope.isPLaying = !1, $scope.justDragged = 99999, $scope.sortableOptions = {
        start: function(e, ui) {
            ui.item.sortable.index === $scope.index && ($scope.isPLaying = !0);
        },
        stop: function(e, ui) {
            $scope.isPLaying && ($scope.index = ui.item.index()), $scope.indexHighLighted = ui.item.index(), 
            $scope.justDragged = ui.item.index(), $scope.isPLaying = !1;
        }
    };
});